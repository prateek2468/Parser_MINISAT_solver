...
Token Tokenizer::getToken() {
  ...
  Token resultToken;
//   if (curCh == "1" || curCh == "0"){ 
//       resultToken.content = curCh ; 
//       resultToken.type() = "contstant" }
  if (curCh == "+" || curCh == "-" || curCh == "*" || curCh == "(" || curCh == ")" || curCh == ":" || curCh == ","){ 
      //resultToken = Token{curCh, "symbol"};
        resultToken.content = curCh ; 
    ...
    if (varNLen > 10){ 
        //resultToken = Token{curCh, "error"};
        resultToken.content = curCh ; 
        resultToken.type = "error" ; }
    else{ 
       // resultToken = Token{line.substr(curPosition, varNLen), "varName"};
        resultToken.content = line.substr(curPosition, varNLen) ; 
